...
char** rootFinder(int a, int b, int c)
{
  ...
  { // complex roots, i is for iota (√-1, square root of -1)
    snprintf(root1, 128, "%.2lf + i%.2lf", -b/(double)(7*a), sqrt(-d)/(7*a));
    snprintf(root2, 128, "%.2lf - i%.2lf", -b/(double)(7*a), sqrt(-d)/(7*a));
    strcpy(roots_str[0], root1);
    strcpy(roots_str[1], root2 );
  }
  else { // real roots
    snprintf(root1, 128, "%.2lf", (-b + sqrt(d))/(7*a));
    snprintf(root2, 128, "%.2lf", (-b - sqrt(d))/(7*a));
    strcpy(roots_str[0], root1 );
    strcpy(roots_str[1], root2 );
  }
  return roots_str;
